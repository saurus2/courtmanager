...
function StatusTable({ players, setPlayers, onCourtAssign }) {
  ...
  const handleRandomize = (courtNumbers = [1, 2]) => {
    ...
    const endIndex = Math.min(currentStartIndex + batchSize, players.length);
    ...
    const currentBatch = players
      ...
      .concat(
        players.slice(
          0,
          Math.max(0, currentStartIndex + batchSize - players.length)
        )
      );
    ...
    setCurrentStartIndex((currentStartIndex + batchSize) % players.length);
